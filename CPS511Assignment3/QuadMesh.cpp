	...
	for (int i = 0; i < this->maxMeshSize + 1; i++)
	{
		for (int j = 0; j < this->maxMeshSize + 1; j++)
		{
			...
			for (int x = 0; x <= blobCount; x++) { //adds height to a vertex from all of the blobs
				...
				vertices[i*(maxMeshSize + 1) + j].position.y += (blobList[x].height)*exp(-((blobList[x].width) * distance));
			}
		}
	}
	this->ComputeNormals();
...
void QuadMesh::assignColor(GLfloat col[3], GLfloat r, GLfloat g, GLfloat b) {
	col[0] = r;
	col[1] = g;
	col[2] = b;
}

int QuadMesh::isEven(int x) {
	if (x % 2 == 0)
		return 1;
	else
		return 0;
...
void QuadMesh::makeTextureMap()
{
	for (int i = 0; i < 64; i++)
		for (int j = 0; j < 64; j++) {
			if (isEven(i / 8 + j / 8)) {
				assignColor(textureMap[i][j], 0.0, 0.3, 1.0);
				assignColor(textureMap2[i][j], 0.0, 1.0, 0.0);
			}
			else {
